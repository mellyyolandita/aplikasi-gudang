...
// jika user sudah login, maka jalankan perintah untuk cetak
  ...
  $no = 1;
  // variabel untuk total barang keluar
  $total_keluar = 0;
  ...
  $query = mysqli_query($mysqli, "SELECT a.id_transaksi, a.tanggal, a.barang, a.penerima, a.harga, a.jumlah, a.total_harga, b.nama_barang, c.nama_satuan
  ...
  while ($data = mysqli_fetch_assoc($query)) {
    ...
    $html .= '		<tr>
                    ...
                    <td width="70" class="text-center">' . $data['id_transaksi'] . '</td>
                    <td width="70" class="text-center">' . date('d-m-Y', strtotime($data['tanggal'])) . '</td>
                    <td width="70" class="text-center">' . $data['nama_barang'] . '</td>
                    <td width="70" class="text-center">' . $data['penerima'] . '</td>
                    <td width="70" class="text-right">Rp. ' . number_format($data['harga'], 0, '', '.') . '</td>
                    <td width="50" class="text-right">' . number_format($data['jumlah'], 0, '', '.') . '</td>
                    <td width="90" class="text-right">Rp. ' . number_format($data['total_harga'], 0, '', '.') . '</td>
                  </tr>';
    // jumlahkan "keluar" untuk mendapatkan "total_keluar"
    $total_keluar += $data['total_harga'];
  }
  // tampilkan data total
  $html .= '			<tr>
                    <td class="text-center font-weight-bold" colspan="7">Total</td>
                    <td class="text-right font-weight-bold">Rp. ' . number_format($total_keluar, 0, '', '.') . '</td>
                  </tr>';
  $html .= '		</tbody>
              </table>
            </div>
